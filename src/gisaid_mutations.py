...
def count_mutations(align, pos_map, ref_seq,
    ...
    def remove_non_nucs(clist):
        ...
        for i in range(len(clist)):
            if clist[i][0] not in [b'A', b'C', b'G', b'T', b'-']:
    ...
    rows = align.seq_count

    align_cols = []
    ref_cols = []
    nts = {b'A': [], b'C': [], b'G': [], b'T': [], b'-': []}
    other = []
    consensus = []
    consensus_pct = []
    alt = []
    ...
    # create a collection of lists containing column information
    for col in range(start, end):          
        c = Counter(align.align_array[:, col])
        if len(c) > 1:
            common = c.most_common()
            
            if common[0][0] in [b'N', b'-']:
            ...
            consens = common[0][0]
            consensus.append(str(consens, 'UTF-8'))
            try:
            ...
            # get alt nucleotide
            alt_nuc = ''
            for nuc, cnt in common:
                if nuc != align.align_array[0, col]:
                    alt_nuc = nuc.decode()
                    break

            ref.append(align.align_array[0, col])
            alt.append(alt_nuc)
...
def find_location_feature(ref_seq, position, alt, consens):
    ...
    dictionary
        ...
        codon_start, codon_end - start and end of codon
        codon - the codon,
...
def count_the_mutations(ref_seq_gb_file,
    ...
    ----------
    ref_seq_gb_file : str
        The name of a GenBank file containing the reference genome.
    ...
    ref_file : str
        ...
        the aligned Wuhan reference sequence.
    ...
    align = gisaid_read_alignment_file(msa_file)
